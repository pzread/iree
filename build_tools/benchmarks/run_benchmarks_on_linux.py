...
from common.benchmark_definition import (PlatformType, DeviceInfo, BenchmarkInfo,
...
LINUX_TMP_DIR = "/tmp/iree-benchmarks"
...
def push_to_tmp_dir(content: str,
                    relative_dir: str = "",
                    verbose: bool = False) -> str:
  """Pushes content onto the device.

  Args:
    content: the full path to the source file.
    relative_dir: the directory to push to; relative to LINUX_TMP_DIR.

  Returns:
    The full path to the content on the device.
  """
  dir_path = os.path.join(LINUX_TMP_DIR, relative_dir)
  if not os.path.exists(dir_path):
    execute_cmd(["mkdir", "-p", dir_path], verbose=verbose)

  filename = os.path.basename(content)
  target_path = os.path.join(dir_path, filename)
  execute_cmd(["cp", os.path.abspath(content), target_path], verbose=verbose)
  return target_path


def device_execute_and_get_output(cmd_args: Sequence[str],
                                  relative_dir: str = "",
                                  verbose: bool = False) -> str:
  ...
  """
  return execute_cmd_and_get_output(cmd_args,
                                    verbose=verbose,
                                    cwd=os.path.join(LINUX_TMP_DIR,
                                                     relative_dir))


def device_execute(cmd_args: Sequence[str],
                   relative_dir: str = "",
  ...
  return execute_cmd(cmd_args,
                     verbose=verbose,
                     cwd=os.path.join(LINUX_TMP_DIR, relative_dir))


def device_execute_as_root(
    cmd_args: Sequence[str]) -> subprocess.CompletedProcess:
  """Executes the given command as root."""
  cmd = ["sudo"]
  cmd.extend(cmd_args)
  return device_execute(cmd)


def device_start_cmd(cmd_args: Sequence[str],
                     relative_dir: str,
...
def push_vmfb_files(benchmark_case_dirs: Sequence[str], root_benchmark_dir: str,
  ...
  for case_dir in benchmark_case_dirs:
    ...
    push_to_tmp_dir(vmfb_path, relative_dir=vmfb_rel_dir, verbose=verbose)
...
def run_benchmarks_for_category(
  ...
  for benchmark_case_dir in benchmark_case_dirs:
    ...
    with open(os.path.join(benchmark_case_dir, MODEL_TOOLFILE_NAME)) as f:
      tool = f.read().strip()
      push_to_tmp_dir(os.path.join(normal_benchmark_tool_dir, tool),
                      relative_dir=NORMAL_TOOL_REL_DIR,
                      verbose=verbose)
      if do_capture:
        push_to_tmp_dir(os.path.join(traced_benchmark_tool_dir, tool),
    ...
    try:
      android_relative_dir = os.path.relpath(benchmark_case_dir,
                                             root_benchmark_dir)
      push_to_tmp_dir(os.path.join(benchmark_case_dir, MODEL_FLAGFILE_NAME),
      ...
      if benchmark_key not in skip_benchmarks:
        ...
        cmd = [
            ...
            os.path.join(LINUX_TMP_DIR, NORMAL_TOOL_REL_DIR, tool),
        ...
        result_json = device_execute_and_get_output(cmd,
                                                    android_relative_dir,
                                                    verbose=verbose)
        ...
        pull_cmd = [
            "cp", "-r",
            os.path.join(LINUX_TMP_DIR, android_relative_dir,
      ...
      if do_capture and benchmark_key not in skip_captures:
        run_cmd = [
            ...
            benchmark_info.deduce_taskset(),
            os.path.join(LINUX_TMP_DIR, TRACED_TOOL_REL_DIR, tool),
            f"--flagfile={MODEL_FLAGFILE_NAME}"
        ]
        ...
        process = device_start_cmd(run_cmd,
                                   android_relative_dir,
                                   verbose=verbose)
...
def get_device_info_from_host(verbose: bool = False):
  return DeviceInfo(PlatformType.LINUX, "Host", "x86-64", "none", "none")


def main(args):
  device_info = get_device_info_from_host()
  if args.verbose:
    print(device_info)
  ...
  execute_cmd_and_get_output(["rm", "-rf", LINUX_TMP_DIR], verbose=args.verbose)

  if not args.no_clean:
    # Clear the benchmark directory on the Android device.
    atexit.register(execute_cmd_and_get_output, ["rm", "-rf", LINUX_TMP_DIR],
