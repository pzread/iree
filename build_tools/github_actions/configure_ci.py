...
# Trailer to disable enforced benchmarking, e.g. on "LLVM integrations" PRs.
DISABLE_ENFORCED_BENCHMARKS_KEY = "disable-enforced-benchmarks"
...
def get_benchmark_presets(trailers: Mapping[str, str], labels: Sequence[str],
  ...
  """
  disable_enforced_benchmarks = DISABLE_ENFORCED_BENCHMARKS_KEY in trailers
  if not is_pr:
    preset_options = ["all"]
    print(f"Using benchmark preset 'all' for non-PR run")
  elif is_llvm_integrate_pr and not disable_enforced_benchmarks:
